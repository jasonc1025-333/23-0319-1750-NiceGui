...
###jwc y for debugOn async def run_javascript(code: str, *,
###jwc y for debugOn                          respond: bool = True, timeout: float = 10.0, check_interval: float = 0.01) -> Optional[str]:
###jwc y try 60 to be safe: async def run_javascript(code: str, *,
###jwc y try 60 to be safe:                          respond: bool = True, timeout: float = 30.0, check_interval: float = 0.01) -> Optional[str]:
async def run_javascript(code: str, *,
                         respond: bool = True, timeout: float = 60.0, check_interval: float = 0.01) -> Optional[str]:
    """Run JavaScript
